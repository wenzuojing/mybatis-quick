...
public enum SqlMethod {
    ...
    DELETE_BY_EXAMPLE("deleteByExample", "根据 example 条件删除记录", "<script>DELETE FROM %s %s</script>"),
    DELETE_BATCH("deleteByIds", "根据ID集合，批量删除数据", "<script>DELETE FROM %s WHERE %s IN (%s)</script>"),
    ...
    UPDATE_SELECTIVE_BY_EXAMPLE("updateSelectiveByExample", "根据 example 条件，更新记录", "<script>UPDATE %s %s %s</script>"),
    ...
    SELECT_ONE_BY_EXAMPLE("selectOneByExample", "查询满足条件一条数据", "<script>SELECT %s FROM %s %s %s LIMIT 1</script>"),
    SELECT_LIST("selectListByExample", "查询满足条件所有数据", "<script>SELECT %s FROM %s %s %s %s </script>"),
    SELECT_PAGE("selectPageByExample", "查询满足条件所有数据", "<script>SELECT %s FROM %s %s %s</script>"),

    COUNT_BY_EXAMPLE("countByExample", "查询总数", "<script>SELECT count(*) FROM %s %s</script>");
